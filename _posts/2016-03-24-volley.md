...
2、如果HttpStack传入为null，则如果系统在 Gingerbread 及之后(即 API Level >= 9)，采用基于 HttpURLConnection 的 HurlStack，如果小于 9，采用基于 HttpClient 的 HttpClientStack。为什么呢？这时因为在 Froyo(2.2) 之前，HttpURLConnection 有个重大 Bug，**调用 close() 函数会影响连接池，导致连接复用失效**:通常的解决办法就是直接禁用掉连接池的功能
```java
